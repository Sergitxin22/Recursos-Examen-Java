public class OrdenacionPorSelección {
    ...
    public static void main(String[] args) {
        ...
        // Vamos a ordenar por el idOficina
        // Por el método de SELECCIÓN ASCENDENTE
        // Ordenamos uno a uno los elementos
        // HASTA EL ANTEÚLTIMO
        for (int i = 0; i < numEmpleados - 1; i++) {
            // El elemento i es el MENOR (ASCENDENTE)
            // hasta el momento
            int menor = i;
            // Vamos a comparar el menor hasta ahora
            // con el resto
            for (int j = i+1; j < numEmpleados; j++) {
                // Si el elemento j es MENOR que el menor hasta ahora
                if (empleados[j].getIdOficina()                    
                    <empleados[menor].getIdOficina()){
                    // El j es el menor
                    menor = j;
                }
            }
            // En menor tenemos la posición del menor elemento
            // desde i hasta el final
            // Ese elemento debe ir en la posición i
            // Intercambiamos i y menor (si el menor no está ya en i)
            if (i != menor){
                Empleado temp = empleados[i];
                empleados[i] = empleados[menor];
                empleados[menor] = temp;
            }
