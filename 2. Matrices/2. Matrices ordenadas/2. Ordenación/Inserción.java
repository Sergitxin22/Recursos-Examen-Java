public class OrdenacionPorInsercion {
    ...
    public static void main(String[] args) {
        ...
        // Vamos a ordenar por el NOMBRE del empleado
        // Por el método de INSERCIÓN
        // Ordenamos uno a uno los elementos
        // HASTA EL ANTEÚLTIMO
        for (int i = 0; i < numEmpleados - 1; i++) {
            // Comparamos cada elemento con el resto
            for (int j = i+1; j < numEmpleados; j++) {
                // Si no están bien ordenados
                // ASCENDENTE
                if (empleados[i].getNombre()
                    .compareToIgnoreCase(
                    empleados[j].getNombre())>0){
                    // Intercambiamos los empleados i y j
